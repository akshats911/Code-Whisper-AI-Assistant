...
export async function promptForApiKey(provider: string, context: vscode.ExtensionContext) {
    if (!supportedProviders.includes(provider.toLowerCase())) {
        vscode.window.showErrorMessage(`Invalid provider "${provider}" in the CodeWhisper settings. Please use a valid provider and restart the extension.`);
        return;
    }
    ...
    const apiKey = await vscode.window.showInputBox({
        prompt: `Enter your ${providerCleanName} API key to use CodeWhisper. Your API key will be stored in VS Code\'s SecretStorage.`,
        ignoreFocusOut: true,
        password: true,
    });

    const secretStorageKey = `codegpt.${provider}ApiKey`;
    ...
    if (apiKey) {
        ...
        vscode.window.showErrorMessage('No API key entered. Please enter your API key to use CodeWhisper.');
    }
}
